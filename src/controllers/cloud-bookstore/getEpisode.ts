...
export default async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const episodeNum = await Episode.findByOnlyEpisodeId(episodeId).then(
      (data) => data.episode_episodeNum,
    );
    ...
    if (userId) {
      ...
      if (episodeNum === 1) {
        ...
        await Episode.findByEpisodeId(episodeId, novelId).then((episode) =>
              ...
              if (purchase.length === 0) {
                ...
                if (cloudBalance.user_cloud === 0) {
                  ...
                  await User.cutCloud(userId) // 구름 1 차감
                    ...
                    .then(() =>
                      // 구름 기록 저장
                      CloudHistory.updateEpisodeCloud(
                        userId,
                        novelId,
                        episodeId,
                      ),
